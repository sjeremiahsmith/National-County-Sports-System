...
requireLogin();
requireRole(['super_admin', 'county_coordinator', 'lofa_admin', 'bong_admin', 'kru_admin', 'gedeh_admin', 'sports_coord']);

if (isCoordViewer()) {
    $_SESSION['error'] = 'You do not have permission to access this page.';
    header('Location: ' . APP_URL . 'pages/players/list.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $formData = compact(['nir_number', 'full_name', 'date_of_birth', 'gender', 'nationality', 'year_of_nscm', 'age', 'city', 'last_club', 'current_club', 'county_id', 'primary_position', 'medical_fitness_status', 'medical_notes', 'sport_discipline_id']);
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    $formData = [
        ...
        'primary_position' => '',
        'medical_fitness_status' => 'pending_review',
...
<form method="POST" action="" enctype="multipart/form-data" class="needs-validation" novalidate>
    <div class="row g-3">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="county_id" class="form-select select2" required>
                                        ...
                                        $groups = ['A', 'B', 'C', 'D'];
                                        foreach ($groups as $g):
                                            $groupCounties = $db->fetchAll("SELECT * FROM counties WHERE group_label = ? ORDER BY name ASC", [$g]);
                                            if (!empty($groupCounties)):
                                        ?>
                                        <optgroup label="Group <?= $g ?>">
                                            <?php foreach ($groupCounties as $c): ?>
        ...
        <div class="col-md-6">
            <div class="card h-100">
                <div class="card-header bg-white">
                    <h5 class="mb-0"><i class="bi bi-heart-pulse me-2"></i>Medical Fitness</h5>
                </div>
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-4">
